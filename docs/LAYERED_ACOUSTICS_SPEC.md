...
**Status: standalone instrument delivered through v0.12; saved SAM ROI integration remains planned.** Version 0.11 implements coherent multilayer spectra, complete-column extraction, property editing, primary/direct comparison and Gaussian single-slab RF. Version 0.12 adds a distinct causal gamma excitation with general multilayer reflected RF, quadrature and envelope, an analytic alias/frequency-cutoff contract, and Arb arithmetic enclosures. Both have immutable report workflows. See [LAYERED_ACOUSTICS.md](LAYERED_ACOUSTICS.md), [CAUSAL_LAYERED_RF.md](CAUSAL_LAYERED_RF.md) and [VERIFICATION.md](VERIFICATION.md) for the selected equations, controls, limits and executed evidence. Prepared 5 September 2026 against v0.10; the wider contract below guides the remaining saved ROI integration. This is not a full three-dimensional elastic solver or an experimentally calibrated model.

The standalone instrument defaults extracted exterior media to water and permits explicitly edited positive real exterior impedance/speed assumptions. Its spectral reference planes exclude standoff; time responses add the explicit incident-medium round trip. Existing saved SAM still uses the primary-echo model. The v0.12 method uses a smooth causal pulse and validated inversion; it does not silently transform the sampled spectrum plot or claim that passivity alone certifies time error. Do not attach this response to saved volume or depth products until the remaining observation and lifecycle contract has passed.
