...
| M4 — X-ray projection volume | General CPU projector, multiangle acquisition, detector controls, view/sinogram browser | Analytical lengths at 0°, 90° and oblique views; no angle singularities; known orientation/fiducial tests; seeded noise statistics and chunk-independent results |
...
Target M1–M4 first: **six HBM sites, an inspectable layered HBM ROI, saved multiangle X-ray projections, a saved SAM RF cube, richer scan recipes, a job monitor and volume slices**. Then complete M5 to obtain xyz X-ray reconstructions and add explicitly labeled SAM depth estimates using the saved RF. This makes the program a volume acquisition workbench with reusable datasets; subsequent fidelity work plugs into that foundation.
