...
The geometry grid has `resolution` samples in x and y, and twice that number in z. Echoes with pre-pulse amplitudes below `1e-8` are omitted. The acoustic acquisition is limited to 12 µs, with a warning when later echoes are excluded. The requested probe snaps to its nearest sampled column; the result retains both the requested coordinates and the sampled center. Numerical warnings identify thin geometry and an undersampled modeled focal spot. Very large resolution/frequency/time-window combinations are rejected by the compute budget rather than silently reducing the requested sampling.
