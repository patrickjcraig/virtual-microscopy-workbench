...
import { VOXEL_PATHS,CONTINUOUS_PATHS,pathModelLabel,pathModelOptions,savedPathModel } from './path-model.js';
...
export class VolumeWorkspace {
  constructor({request,getSnapshot,onRecipes,onCausal}) {
    Object.assign(this,{request,getSnapshot,onRecipes,onCausal});this.jobs=[];this.datasets=[];this.ceiling=.5;this.cursor={x_index:0,y_index:0,time_index:0};this.gate=null;
        ...
        <div class="vol-layout">
          <aside class="vol-acquisition" aria-label="Volume acquisition and saved datasets">
            <details id="vol-acquire-panel" open><summary>New SAM volume</summary>
              ...
              <button id="vol-causal-btn" class="small" type="button">Causal multilayer · independent columns</button><button id="vol-recipes-btn" class="small" type="button">Use these settings in recipes & cases</button><form id="vol-form"><div class="vol-path-choice"><label for="vol-path_model">Material paths<select id="vol-path_model" aria-describedby="vol-path-note">${pathModelOptions}</select></label><p id="vol-path-note" class="vol-hint"></p></div>
    ...
    $('#vol-causal-btn').onclick=()=>{$('#vol-dialog').close();this.onCausal?.();};
  ...
  capture(){
    ...
    $('#vol-path_model').value=s.path_model ?? VOXEL_PATHS;$('#vol-sample_rate_mhz').value=Math.max(400,8*(s.frequency_mhz || 50));$('#vol-focus_mm').max=snapshot.twin.size_mm[2];
  ...
  acquisition(){const fields=['scan_nx','scan_ny','depth_samples','frequency_mhz','fractional_bandwidth','focus_mm','water_standoff_mm','record_start_us','record_duration_us','sample_rate_mhz'];return {path_model:$('#vol-path_model').value,...Object.fromEntries(fields.map(key=>[key,Number($(`#vol-${key}`).value)])),include_defects:this.snapshot.settings?.include_defects!==false,...(this.snapshot.settings?.roi_mm?{roi_mm:[...this.snapshot.settings.roi_mm]}:{})};}
  ...
  changed(){
    const continuous=$('#vol-path_model').value===CONTINUOUS_PATHS;$('#vol-depth_samples').disabled=continuous;$('#vol-depth-state').textContent=continuous?'(inactive)':'';$('#vol-path-note').textContent=continuous?'Continuous vertical intersections use authored boundaries. The depth-sample value is retained in the recipe but is inactive. XY sampling, material assumptions, blur and RF sampling remain.':'Material occupancy is sampled at voxel centers; the geometry depth setting controls the Z grid. Both path methods use normal-incidence SAM.';
  ...
  async estimate(){
    ...
    try{const estimate=await this.request('/api/v2/estimate',payload,controller.signal);if(controller!==this.estimateController || key!==JSON.stringify(this.payload()))return;this.estimateKey=key;this.estimateResult=estimate;
      $('#vol-estimate').textContent=`${pathModelLabel(estimate.path_model ?? payload.acquisition.path_model)}. ${estimate.depth_samples_used===false || payload.acquisition.path_model===CONTINUOUS_PATHS?'No Z voxel grid; depth samples are inactive. ':''}${estimate.shape.join(' × ')} samples (y, x, time). RF ${humanBytes(estimate.rf_bytes)} + envelope ${humanBytes(estimate.envelope_bytes)}. Storage ${humanBytes(estimate.total_bytes)}; estimated peak memory ${humanBytes(estimate.estimated_peak_bytes)}.${estimate.free_disk_bytes!==undefined?` Disk available: ${humanBytes(estimate.free_disk_bytes)}.`:''}${Array.isArray(estimate.padded_shape_yx)?` Padded XY columns: ${estimate.padded_shape_yx.join(' × ')} (Y, X).`:''}${Number.isFinite(estimate.path_candidate_tests)?` Candidate tests: ${estimate.path_candidate_tests.toLocaleString()}.`:''}${Number.isFinite(estimate.path_event_work_units)?` Event work units: ${estimate.path_event_work_units.toLocaleString()}.`:''}`;
  ...
  renderDatasets(){
    ...
    for(const manifest of [...this.datasets].sort((a,b)=>String(b.created_at || '').localeCompare(a.created_at || ''))){const button=document.createElement('button'),id=manifest.dataset_id || manifest.id;button.className='vol-dataset';button.dataset.datasetId=id;button.disabled=!complete(manifest);button.setAttribute('aria-pressed',String(id===(this.manifest?.dataset_id || this.manifest?.id)));const name=document.createElement('strong');name.textContent=manifest.name || 'Acoustic volume';const metadata=document.createElement('span');metadata.textContent=`${(manifest.shape || []).join(' × ')} · ${manifest.state || manifest.status || (complete(manifest)?'complete':'incomplete')} · ${pathModelLabel(savedPathModel(manifest))}`;const date=document.createElement('span');date.textContent=`${new Date(manifest.created_at).toLocaleString()} · ${String(id).slice(0,8)}`;button.append(name,metadata,date);button.addEventListener('click',()=>this.selectDataset(id));list.append(button);}
  }
  async selectDataset(id){
    ...
    try{const manifest=await this.request(`/api/v2/datasets/${encodeURIComponent(id)}`,null,controller.signal);if(controller!==this.selectionController || !$('#vol-dialog').open)return;if(!complete(manifest)){this.message('This dataset is incomplete. Resume its acquisition to inspect the completed volume.');return;}
      ...
      const pathModel=savedPathModel(manifest);$('#vol-path-summary').dataset.pathModel=pathModel;$('#vol-path-summary').textContent=`${pathModelLabel(pathModel)}. ${pathModel===CONTINUOUS_PATHS?'Authored interface depths; no Z voxel grid. The recorded depth-sample value was inactive.':'Material paths sampled at voxel centers.'} Saved RF is unchanged by the current acquisition controls.`;
      $('#vol-provenance').textContent=`Dataset ${id}. Synthetic, uncalibrated relative amplitudes. The frozen specimen and acquisition recipe are included in the archive. The source RF is retained; gates and display windows change only the derived view. Time is not depth; any depth mapping requires its own declared velocity model.`;
