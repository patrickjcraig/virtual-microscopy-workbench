...
async function openCausalVolumes(options){try{causalWorkspacePromise ||= import('./causal-volumes.js').then(({CausalVolumeWorkspace})=>new CausalVolumeWorkspace({request,getSnapshot:()=>({twin:state.twin,settings:settingsFromControls()})}));(await causalWorkspacePromise).open(options);}catch(error){causalWorkspacePromise=null;notify(`Unable to open causal volumes: ${error.message}`,true);}}
...
$('#export-results').addEventListener('click',()=>{if(state.result)downloadJSON({...state.result,export_metadata:{application:'Virtual microscopy workbench',version:'0.13.0',exported_at:new Date().toISOString(),display_windows:{xray:[0,1],sam:[0,state.samCeiling],bscan:[0,1]},settings_changed_since_acquisition:state.stale}},`${safeName(state.result.twin.name)}-acquisition-${safeName(state.result.run_id || new Date().toISOString())}.json`);});
