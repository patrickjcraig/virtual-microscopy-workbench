...
app.innerHTML = `
  <header class="topbar">
    ...
    <div class="top-actions"><span class="mode-label"><i></i>Synthetic forward model</span><button id="volumes-btn" title="Saved acoustic RF volumes">${icon('layers')}Saved volumes</button><button id="causal-volumes-btn" title="Saved causal multilayer independent-column volumes">${icon('acoustic')}Causal volumes</button><button id="observations-btn" title="Finite coherent spatial filter of saved causal volumes">${icon('sliders')}Spatial filter</button><button id="observation-comparisons-btn" title="Compare saved finite coherent observations">${icon('sliders')}Observation comparisons</button><button id="causal-comparisons-btn" title="Compare saved causal responses and residual bounds">${icon('sliders')}Causal comparisons</button><button id="recipes-btn">${icon('sliders')}Recipes & comparisons</button><button id="xray-recipes-btn">${icon('sliders')}X-ray recipes</button><button id="xray-volumes-btn">${icon('xray')}X-ray volumes</button><button id="reconstruction-btn">${icon('layers')}CT reconstruction</button><button id="layered-acoustics-btn">${icon('acoustic')}Layered acoustics</button><button id="sls-acoustics-btn" title="Manual scalar single-relaxation material analysis">${icon('acoustic')}SLS materials</button><button id="mixed-acoustics-btn" title="Standalone finite lossless real and scalar SLS media">${icon('acoustic')}Mixed finite media</button><button id="material-assignments-btn" title="Explicit material assumptions and full-depth geometric coverage">${icon('layers')}Material assignments</button><button id="sls-comparisons-btn" title="Compare saved scalar SLS material responses">${icon('sliders')}SLS comparisons</button><button id="sam-depth-btn">${icon('acoustic')}SAM depth</button><button id="assumptions-btn">${icon('info')}Model & assumptions</button><button id="export-results" disabled>${icon('download')}Export acquisition</button></div>
  </header>
...
async function openObservations(options){try{observationWorkspacePromise ||= import('./observations.js').then(({ObservationWorkspace})=>new ObservationWorkspace({request,onCompare:options=>openObservationComparisons(options)}));await (await observationWorkspacePromise).open(options);}catch(error){observationWorkspacePromise=null;notify(`Unable to open finite coherent spatial filter: ${error.message}`,true);}}
$('#observations-btn').addEventListener('click',()=>openObservations());
let observationComparisonPromise;
async function openObservationComparisons(options){try{observationComparisonPromise ||= import('./observation-comparisons.js').then(({ObservationComparisonWorkspace})=>new ObservationComparisonWorkspace({request}));await (await observationComparisonPromise).open(options);}catch(error){observationComparisonPromise=null;notify(`Unable to open observation comparisons: ${error.message}`,true);}}
$('#observation-comparisons-btn').addEventListener('click',()=>openObservationComparisons());
...
let slsComparisonPromise;
async function openSLSComparisons(options){try{slsComparisonPromise ||= import('./sls-comparisons.js').then(({SLSComparisonsWorkspace})=>new SLSComparisonsWorkspace({request}));await (await slsComparisonPromise).open(options);}catch(error){slsComparisonPromise=null;notify(`Unable to open SLS comparisons: ${error.message}`,true);}}
$('#sls-comparisons-btn').addEventListener('click',()=>openSLSComparisons());
let mixedAcousticsPromise;
$('#mixed-acoustics-btn').addEventListener('click',async()=>{try{mixedAcousticsPromise ||= import('./mixed-acoustics.js').then(({MixedAcousticsWorkspace})=>new MixedAcousticsWorkspace({request}));await (await mixedAcousticsPromise).open();}catch(error){mixedAcousticsPromise=null;notify(`Unable to open mixed finite media: ${error.message}`,true);}});
let materialAssignmentsPromise;
$('#material-assignments-btn').addEventListener('click',async()=>{try{materialAssignmentsPromise ||= import('./material-assignments.js').then(({MaterialAssignmentsWorkspace})=>new MaterialAssignmentsWorkspace({request,getSnapshot:()=>({twin:state.twin,settings:settingsFromControls()})}));await (await materialAssignmentsPromise).open();}catch(error){materialAssignmentsPromise=null;notify(`Unable to open material assignments: ${error.message}`,true);}});
let slsAcousticsPromise;
$('#sls-acoustics-btn').addEventListener('click',async()=>{try{slsAcousticsPromise ||= import('./sls-acoustics.js').then(({SLSAcousticsWorkspace})=>new SLSAcousticsWorkspace({request,onCompare:id=>openSLSComparisons({referenceReportId:id})}));await (await slsAcousticsPromise).open();}catch(error){slsAcousticsPromise=null;notify(`Unable to open scalar SLS acoustics: ${error.message}`,true);}});
