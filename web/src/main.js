...
$('#sls-acoustics-btn').addEventListener('click',async()=>{try{slsAcousticsPromise ||= import('./sls-acoustics.js').then(({SLSAcousticsWorkspace})=>new SLSAcousticsWorkspace({request}));await (await slsAcousticsPromise).open();}catch(error){slsAcousticsPromise=null;notify(`Unable to open scalar SLS acoustics: ${error.message}`,true);}});
