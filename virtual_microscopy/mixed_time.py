"""Bounded causal gamma reflection from passive typed real/SLS layers using Arb balls.
...
The new material law and identity preserve every earlier forward model.
The alias/cutoff contract is derived in docs/MIXED_MATERIAL_PROOF.md; only
reflected pressure is contractive at the real exterior port.
"""
...
from .mixed_schemas import MixedStack as LayeredStack
from .mixed_acoustics import _scattering, MATERIAL_MODEL_VERSION, layer_counts
from .layered_time import _ARITHMETIC_LOCK

MODEL_VERSION = "scalar-mixed-causal-gamma-arb-0.20.0"
CERTIFICATE_VERSION = "scalar-mixed-reflected-gamma-1"
MAX_FREQUENCY_TERMS = 16385
MAX_TIME_SAMPLES = 2049
MAX_LAYER_FREQUENCY_WORK = 2_000_000
...
def _validate(stack, time_us, settings):
    ...
    if not isinstance(time_us, (list, tuple)) or not 2 <= len(time_us) <= MAX_TIME_SAMPLES:
        raise ValueError("Causal mixed RF requires 2 to 2,049 actual recorded time centers.")
    ...
    if max(b-a for a,b in zip(time,time[1:]))*options["center_frequency_mhz"] > (1/8)*(1+1e-10):
        raise ValueError("Causal mixed RF requires at least eight samples per carrier period.")
...
def _plan(stack, time, options):
    ...
    layer_work, inverse_work = count*(12*nl+8), count*nt
    peak_bytes = 32*1024**2+count*4096+nt*4096+nl*16384
    if count > MAX_FREQUENCY_TERMS or layer_work > MAX_LAYER_FREQUENCY_WORK or inverse_work > MAX_INVERSE_WORK or peak_bytes > MAX_PEAK_BYTES:
        raise ValueError("Causal mixed RF exceeds its frequency/work/workspace budget. Reduce the recording or change explicit pulse/tolerance settings; layers and error targets were not silently changed.")
    return dict(omega=omega,rate=rate,peak=peak,numerator=numerator,fourier_l1=fourier_l1,
                ...
                peak_bytes=peak_bytes,counts=layer_counts(stack))


def _estimate_fields(plan, options, nt):
    return {"model_version": MODEL_VERSION, "certificate_version": CERTIFICATE_VERSION,
            "material_model_version": MATERIAL_MODEL_VERSION, **plan["counts"],
            "material_frequency_work_units": plan["layer_work"], "frequency_terms": plan["count"], "time_samples": nt,
...
def causal_gamma_response(stack, time_us, settings):
    ...
    with _ARITHMETIC_LOCK, ctx.workprec(options["precision_bits"]):
        ...
        for k in range(-p["half_count"],p["half_count"]+1):
            ...
            reflection, _, lower = _scattering(stack, s, transmission=False)
        ...
        for actual_time in time:
            ...
            if not all(isfinite(v) for v in (real,imag,env)):
                raise ValueError("The causal mixed response produced a nonfinite field.")
        ...
        components = {"analytic_alias_bound": _upper_float(p["alias"]),
            "frequency_cutoff_bound": _upper_float(p["cutoff"]),
            "arithmetic_complex_bound": _upper_float(max_complex_error),
            "arithmetic_envelope_bound": _upper_float(max_envelope_error)}
        # Compose the already-published components as exact represented doubles.
        # This is conservative for both complex and separately saved magnitude.
        total = sum((arb(value) for value in components.values()), arb(0))
        ...
        diagnostics.update(arithmetic_status="Accepted: every returned complex-pressure and envelope sample is covered by the saved global bound.",
            **components,
            ...
            certificate_definition="For the explicit typed real/SLS stack and gamma pulse at each saved time, reflected complex-pressure and magnitude error <= the outward sum of the published alias, cutoff, maximal Arb-to-output complex and magnitude components. Alias C0/(exp(sigma*T)-1) applies only for 0<=t<T; cutoff exp(sigma*b)*D/(pi*m*(K*delta)^m). All represented inputs, exact SI conversions, typed real Z/c propagation and SLS roots, complex interfaces, denominators, propagation, pulse constants, phases and blocked inverse sums use Arb enclosures; conversion is bounded against the actual returned doubles.",
            evidence_status="Reflected-response numerical enclosure for exact represented input values under the passive typed lossless-real/single-relaxation scalar model; excludes material, geometry and measured-instrument uncertainty. No transmitted RF, saved SAM volume or depth product is created.")
