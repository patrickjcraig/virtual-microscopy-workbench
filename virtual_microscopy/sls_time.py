"""Bounded causal gamma reflection from passive SLS layers using Arb balls.
...
The alias/cutoff contract is derived in docs/SLS_MATERIAL_PROOF.md; only
...
from .sls_schemas import SLSStack as LayeredStack
from .sls_acoustics import _scattering, MATERIAL_MODEL_VERSION
...
def _validate(stack, time_us, settings):
    ...
    if not isinstance(time_us, (list, tuple)) or not 2 <= len(time_us) <= MAX_TIME_SAMPLES:
        raise ValueError("Causal SLS RF requires 2 to 2,049 actual recorded time centers.")
    ...
    if max(b-a for a,b in zip(time,time[1:]))*options["center_frequency_mhz"] > (1/8)*(1+1e-10):
        raise ValueError("Causal SLS RF requires at least eight samples per carrier period.")
...
def _plan(stack, time, options):
    ...
    if count > MAX_FREQUENCY_TERMS or layer_work > MAX_LAYER_FREQUENCY_WORK or inverse_work > MAX_INVERSE_WORK or peak_bytes > MAX_PEAK_BYTES:
        raise ValueError("Causal SLS RF exceeds its frequency/work/workspace budget. Reduce the recording or change explicit pulse/tolerance settings; layers and error targets were not silently changed.")
    ...
    with _ARITHMETIC_LOCK, ctx.workprec(options["precision_bits"]):
        ...
        for actual_time in time:
            ...
            if not all(isfinite(v) for v in (real,imag,env)):
                raise ValueError("The causal SLS response produced a nonfinite field.")
        ...
        diagnostics.update(arithmetic_status="Accepted: every returned complex-pressure and envelope sample is covered by the saved global bound.",
            **components,
            ...
            evidence_status="Reflected-response numerical enclosure for exact represented input values under the passive single-relaxation scalar model; excludes material, geometry and measured-instrument uncertainty. No transmitted RF, saved SAM volume or depth product is created.")
