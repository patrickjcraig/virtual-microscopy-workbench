"""Explicit comparisons of completed finite coherent observations."""
...
class ObservationComparisonRequest(StrictModel):
    name: str = Field(default="", max_length=160)
    reference_dataset_id: str
    candidate_dataset_id: str
    policy: Literal["same_observation_and_excitation_v1"] = "same_observation_and_excitation_v1"
    gate_start_us: StrictFloat = Field(ge=0, le=12)
    gate_end_us: StrictFloat = Field(ge=0, le=12)
    x_index: int | None = Field(default=None, ge=0, le=61, strict=True)
    y_index: int | None = Field(default=None, ge=0, le=61, strict=True)
    ...
    def ordered_gate(self):
        if self.gate_end_us <= self.gate_start_us:
            raise ValueError("An observation comparison requires finite gate start < end.")
        return self
