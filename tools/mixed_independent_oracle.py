"""Independent mixed real/SLS inverse, never a numerical certificate.
...
def frequency_transfer(stack, frequency_mhz):
    """Bounded independent p/v spectra for moderate verification fixtures.

    Outputs are high-precision decimal strings. This is ordinary cross-method
    agreement, not an interval enclosure or frequency-error certificate.
    """
    if not isinstance(frequency_mhz, (list, tuple)) or not 1 <= len(frequency_mhz) <= 513:
        raise ValueError("Independent spectra support 1 to 513 selected frequencies.")
    if any(isinstance(f, bool) or not isfinite(f) or not 0 <= f <= 300 for f in frequency_mhz):
        raise ValueError("Independent spectra require finite frequencies from 0 to 300 MHz.")
    if not 0 <= len(stack['layers']) <= 8:
        raise ValueError("Independent spectra support at most eight finite layers.")
    ...
    values = []
    for frequency in frequency_mhz:
        s = ctx.j*2*ctx.pi*ctx.mpf(frequency)*1_000_000
        q = ctx.eye(2)
        for layer in stack['layers']:
            d = ctx.mpf(layer['thickness_mm'])/1000
            if not d:
                continue
            if layer['kind'] == 'lossless_real':
                z = ctx.mpf(layer['impedance_mrayl'])*1_000_000
                c = ctx.mpf(layer['sound_speed_m_s'])
                rho, modulus = z/c, z*c
            elif layer['kind'] == 'sls':
                rho = ctx.mpf(layer['density_kg_m3'])
                m0 = ctx.mpf(layer['relaxed_modulus_gpa'])*1_000_000_000
                mi = ctx.mpf(layer['unrelaxed_modulus_gpa'])*1_000_000_000
                tau = ctx.mpf(layer['relaxation_time_us'])/1_000_000
                modulus = (m0+mi*tau*s)/(1+tau*s)
            else:
                raise ValueError("An explicit supported finite material kind is required.")
            a = ctx.matrix([[0,-rho*s*d/zi],[-zi*s*d/modulus,0]])
            if max(abs(a[0,1]),abs(a[1,0])) > 500:
                raise ValueError("Independent spectrum fixture is too ill-conditioned.")
            q = ctx.expm(a)*q
        w0, w1 = q[0,0]-zt/zi*q[1,0], q[0,1]-zt/zi*q[1,1]
        r = -(w0+w1)/(w0-w1)
        t = q[0,0]*(1+r)+q[0,1]*(1-r)
        values.append({'frequency_mhz':frequency,
            'reflection':{'real':ctx.nstr(r.real,65),'imaginary':ctx.nstr(r.imag,65)},
            'transmission':{'real':ctx.nstr(t.real,65),'imaginary':ctx.nstr(t.imag,65)}})
    return {'method':'mixed-independent-pressure-velocity-spectrum', 'decimal_precision':80,
            'values':values,'scope':'Ordinary independent numerical agreement; no interval or physical-accuracy certificate.'}
...
def inverse_transfer(stack, settings, time_us, *, degree=64):
    ...
    def material_transfer(s):
        ...
        for layer in stack['layers']:
            ...
            if layer['kind'] == 'lossless_real':
                impedance = ctx.mpf(layer['impedance_mrayl']) * 1_000_000
                speed = ctx.mpf(layer['sound_speed_m_s'])
                density, modulus = impedance/speed, impedance*speed
            elif layer['kind'] == 'sls':
                ...
                tau = ctx.mpf(layer['relaxation_time_us']) / 1_000_000
                # Factored constitutive form, independent of a scattering root.
    ...
    return {'method': 'mixed-mpmath-dehoog-independent-pressure-velocity-transfer',
