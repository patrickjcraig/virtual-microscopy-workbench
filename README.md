...
Version **0.17** adds a separate [scalar relaxation material instrument](docs/SLS_ACOUSTICS.md). Author up to eight layers using density, relaxed/unrelaxed longitudinal modulus, relaxation time and thickness; inspect their attenuation, dispersion and complex impedance; and save a reflected causal gamma response with numerical error bounds. The manually assumed SLS materials are distinct from calibrated HBM properties. Existing [observation comparisons](docs/OBSERVATION_COMPARISONS.md), [finite coherent observations](docs/COHERENT_OBSERVATIONS.md), [causal comparisons](docs/CAUSAL_COMPARISONS.md), [causal volumes](docs/CAUSAL_SAM_VOLUMES.md), primary SAM, Gaussian slab, [X-ray comparisons](docs/XRAY_COMPARISONS.md) and [SAM comparisons](docs/ACQUISITION_COMPARISONS.md) retain their original model identities and saved data.
