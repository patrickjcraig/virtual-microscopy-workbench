...
Version **0.15** adds [finite coherent spatial observations](docs/COHERENT_OBSERVATIONS.md): derive a saved volume by mixing nine neighboring complex SAM responses, inspect its supported interior, and retain separate source, arithmetic and total error bounds. Jobs support cancellation/resume; completed time views and typed exports reopen without the parent dataset. The fixed discrete filter has explicit support and does not represent a calibrated microscope beam. [Saved causal comparisons](docs/CAUSAL_COMPARISONS.md), [causal volumes](docs/CAUSAL_SAM_VOLUMES.md), primary SAM, Gaussian slab, [X-ray comparisons](docs/XRAY_COMPARISONS.md) and [SAM comparisons](docs/ACQUISITION_COMPARISONS.md) remain available.
