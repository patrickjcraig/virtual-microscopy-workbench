...
Version **0.16** adds [comparisons of saved coherent observations](docs/OBSERVATION_COMPARISONS.md): compare compatible intact/defect or numerical-control volumes, inspect linked A/B/residual views, and retain six separate source, subtraction and total bound maps. Immutable reports preserve the gate, ordinary metrics and complete deduplicated source provenance; their initial views and lossless exports reopen without source arrays. [Finite coherent spatial observations](docs/COHERENT_OBSERVATIONS.md), [saved causal comparisons](docs/CAUSAL_COMPARISONS.md), [causal volumes](docs/CAUSAL_SAM_VOLUMES.md), primary SAM, Gaussian slab, [X-ray comparisons](docs/XRAY_COMPARISONS.md) and [SAM comparisons](docs/ACQUISITION_COMPARISONS.md) remain available. The finite observation filter is a declared discrete operator, without experimental beam calibration.
