...
import virtual_microscopy.sls_analysis as analysis
import virtual_microscopy.sls_reports as storage
from virtual_microscopy.sls_reports import SLSReportStore
from virtual_microscopy.sls_schemas import SLSAnalysisRequest


def request(*, causal=True):
    medium = {"name": "Nominal water", "impedance_mrayl": 1.48, "sound_speed_m_s": 1480.}
    return {"name": "Assumed \"slab\" Δ", "stack": {"incident": medium, "terminal": medium,
        "layers": [{"name": "A", "thickness_mm": .05, "density_kg_m3": 1000.,
...
def client(root):
    from virtual_microscopy.sls_api import router
...
def fixture(tmp_path, monkeypatch):
    body = SLSAnalysisRequest.model_validate(request())
    result = analysis.analyze_sls(body)
    calls = []
    def calculate(*args): calls.append("calculate"); return deepcopy(result)
    monkeypatch.setattr(analysis, "analyze_sls", calculate)
    monkeypatch.setattr(analysis, "estimate_sls", lambda *args: deepcopy(result["resources"]))
    monkeypatch.setattr(storage, "_fingerprints", lambda: {name: "1"*64 for name in storage.IMPLEMENTATION_FILES})
    monkeypatch.setattr(storage, "_proof_identity", lambda: {"path": "docs/SLS_MATERIAL_PROOF.md", "sha256": "2"*64})
    return SLSReportStore(tmp_path), body, result, calls
...
def test_real_roundtrip_freezes_exact_units_spectrum_and_reflected_certificate(tmp_path):
    body = request()
    report = SLSReportStore(tmp_path).create(body)
    normalized = SLSAnalysisRequest.model_validate(body).model_dump(mode="json")
    ...
    assert report["kind"] == "sls_layered_analysis" and report["source_status"] == "manual_assumptions"
    ...
    assert SLSReportStore(tmp_path).read(report["id"]) == report
    raw = storage.bounded_payload(report)
    csv_rows = csv.DictReader(io.StringIO(storage.sls_report_csv(report)))
    ...
    assert list(tmp_path.iterdir()) == [tmp_path/"sls-reports"]
...
def test_frequency_only_zero_layer_and_zero_thickness_keep_authored_materials(tmp_path):
    ...
    body["stack"]["layers"] = []
    empty = SLSReportStore(tmp_path).create(body)
...
def test_actual_binary64_time_centers_reach_plan_and_response(monkeypatch):
    from virtual_microscopy import sls_time
    ...
    calls = []
    original_estimate, original_response = sls_time.estimate_causal_gamma, sls_time.causal_gamma_response
    ...
    monkeypatch.setattr(sls_time, "estimate_causal_gamma", preflight); monkeypatch.setattr(sls_time, "causal_gamma_response", synthesize)
    result = analysis.analyze_sls(body)
    assert len(calls) == 2
...
def test_large_combined_request_rejected_before_spectrum_or_rf_synthesis(monkeypatch):
    from virtual_microscopy import sls_acoustics, sls_time
    ...
    monkeypatch.setattr(sls_acoustics, "reflection_spectrum", forbidden)
    monkeypatch.setattr(sls_time, "causal_gamma_response", forbidden)
    with pytest.raises(ValueError, match="budget|workspace|limit"): analysis.analyze_sls(body)
...
def test_historical_reader_avoids_current_schema_solver_proof_and_flint(fixture, monkeypatch):
    ...
    def guarded(name, *args, **kwargs):
        if name == "flint" or name.endswith(("sls_analysis", "sls_acoustics", "sls_time", "sls_schemas")):
            pytest.fail("Historical read imported a current solver/schema")
        return original(name, *args, **kwargs)
    monkeypatch.setattr(builtins, "__import__", guarded)
    assert SLSReportStore(store.root).read(report["id"]) == report
    assert storage.sls_report_csv(report)
    assert SLSReportStore(store.root).list()[0]["id"] == report["id"]
...
def test_resealed_malformed_provenance_rejected_without_current_lookup(fixture, target):
    ...
    if target == "missing_fingerprint": value["implementation_sha256"].pop("sls_time.py")
    elif target == "bad_fingerprint": value["implementation_sha256"]["sls_time.py"] = "z"*64
...
def test_canonical_uuid_and_path_guard(tmp_path, identifier):
    with pytest.raises(ValueError): SLSReportStore(tmp_path).read(identifier)
...
def test_links_rejected_before_parse(tmp_path, monkeypatch, target):
    ...
    store, identifier = SLSReportStore(root), str(uuid4())
...
def test_catalog_is_paginated_and_preserves_unpublished_stages(fixture, monkeypatch):
    ...
    def read(identifier): seen.append(identifier); return original(identifier)
...
def test_catalog_validation(tmp_path, params):
    with pytest.raises(ValueError, match="pagination"): SLSReportStore(tmp_path).list(**params)
...
def test_new_identity_covers_kernel_units_shared_lock_and_proof():
    ...
    assert {"sls_acoustics.py", "sls_time.py", "sls_schemas.py", "layered_time.py", "schemas.py"} <= storage._fingerprints().keys()
...
def test_api_estimate_create_list_exact_exports_and_restart_preserve_unrelated_files(tmp_path):
    ...
    with client(tmp_path) as api:
        assert api.post("/api/v2/sls-acoustics/estimate", json=request()).status_code == 200
        assert not (tmp_path/"sls-reports").exists()
        response = api.post("/api/v2/sls-acoustics/reports", json=request())
        assert response.status_code == 201, response.text
        report = response.json(); prefix = f'/api/v2/sls-acoustics/reports/{report["id"]}'
        ...
        page = api.get("/api/v2/sls-acoustics/reports?limit=1").json()
        assert page["reports"][0]["id"] == report["id"] and page["order"] == "id_desc"
        assert api.get(prefix+"/export?format=png").status_code == 422
        assert api.get("/api/v2/sls-acoustics/reports/not-a-uuid").status_code == 422
        assert api.get(f"/api/v2/sls-acoustics/reports/{uuid4()}").status_code == 404
...
def test_api_rejects_wrong_instrument_or_material_fields_without_publication(tmp_path, change):
    ...
    with client(tmp_path) as api:
        assert api.post("/api/v2/sls-acoustics/reports", json=body).status_code == 422
        assert api.get("/api/v2/sls-acoustics/reports").json()["reports"] == []
    assert not list(tmp_path.iterdir())


def test_service_keeps_calculation_and_serialization_inside_shared_processing_lock(tmp_path, monkeypatch):
    import virtual_microscopy.sls_api as api_module
    entered = []
    class Lock:
        active = False
    ...
    original_estimate, original_payload = analysis.estimate_sls, api_module.bounded_payload
    ...
    monkeypatch.setattr(analysis, "estimate_sls", estimate); monkeypatch.setattr(api_module, "bounded_payload", encode)
    with client(tmp_path) as api:
        assert api.post("/api/v2/sls-acoustics/estimate", json=request(causal=False)).status_code == 200
